...
func make_map(_path: String):
	map_path = _path
	map_scene = load(_path).instantiate()
	add_child(map_scene)
	# var map_image = Image.new()
	# map_image.load(map_scene)
...
func set_player(player: Structures.Player):
	...
	map_scene.position_relative.add_child(player_node)
	player_node.set_player(player)
	var spawn_color_rects = map_scene.player_spawnpoints.get_children()
	var spawn_position = spawn_color_rects[randi() % len(spawn_color_rects)].position
	# var spawn_positions = MapDetails.MAP_DATA[map_path]["Spawnpoints"]
	# var spawn_position = spawn_positions[randi() % len(spawn_positions)]
	player_node.position = Vector2(spawn_position["x"], spawn_position["y"])
	map_scene.player_spawnpoints.queue_free()
...
func place_enemies(difficulty: int):
	...
	for area in map_scene.enemy_spawnpoints.get_children():
		# var area_copy = area.duplicate()
		var num_enemies = randi() % (max_enemies - min_enemies + 1) + min_enemies
		if num_enemies > area.get_child_count():
			num_enemies = area.get_child_count()
		var positions_available = area.get_child_count()
		for i in range(num_enemies):
			...
			var child = area.get_children()[pos_index]
			var pos_data = Vector2(child.position)
			enemy_node.position = Vector2(pos_data["x"], pos_data["y"])
			# temp
			# enemy_node.position = Vector2(22, 8) * MAP_SCALE * MAP_TILE_SIZE * UNKNOWN_ENEMY_POS_SCALE
			enemy_node.pathfinding_target = player_node
			# enemy_node.timer.start()
			map_scene.enemies.add_child(enemy_node)
	...
	map_scene.enemy_spawnpoints.queue_free()
