extends Node

const MAP_DATA: Dictionary = {
    "res://Scenes/Maps/Dungeon.tscn": {
        "name": "Dungeon",
        # "description": "A dark and eerie dungeon filled with unknown dangers.",
        # "size": Vector2(64, 64),
        # "collisions": {
        #     "rectangles": [
        #         {"width": 1, "height": 1},
        #         {"width": 1, "height": 2},
        #         {"width": 1, "height": 3},
        #         {"width": 1, "height": 4},
        #         {"width": 1, "height": 5},
        #         {"width": 1, "height": 6},
        #         {"width": 1, "height": 7},
        #         {"width": 1, "height": 8},
        #         {"width": 1, "height": 9},
        #         {"width": 1, "height": 10},
        #         {"width": 1, "height": 11},
        #         {"width": 1, "height": 12},
        #         {"width": 1, "height": 13},
        #         {"width": 1, "height": 14},
        #         {"width": 1, "height": 15},
        #         {"width": 1, "height": 16},
        #         {"width": 1, "height": 17},
        #         {"width": 1, "height": 18},
        #         {"width": 1, "height": 19},
        #         {"width": 1, "height": 20},
        #         {"width": 1, "height": 23},
        #         {"width": 1, "height": 32},
        #         {"width": 1, "height": 35},
        #         {"width": 2, "height": 1},
        #         {"width": 2, "height": 2},
        #         {"width": 2, "height": 3},
        #         {"width": 2, "height": 4},
        #         {"width": 2, "height": 5},
        #         {"width": 2, "height": 6},
        #         {"width": 2, "height": 8},
        #         {"width": 3, "height": 1},
        #         {"width": 3, "height": 2},
        #         {"width": 4, "height": 1},
        #         {"width": 5, "height": 1},
        #         {"width": 6, "height": 1},
        #         {"width": 6, "height": 2},
        #         {"width": 7, "height": 1},
        #         {"width": 7, "height": 2},
        #         {"width": 8, "height": 1},
        #         {"width": 9, "height": 1},
        #         {"width": 10, "height": 1},
        #         {"width": 11, "height": 1},
        #         {"width": 11, "height": 2},
        #         {"width": 12, "height": 1},
        #         {"width": 13, "height": 1},
        #         {"width": 14, "height": 1},
        #         {"width": 15, "height": 1},
        #         {"width": 16, "height": 1},
        #         {"width": 17, "height": 1},
        #         {"width": 18, "height": 1},
        #         {"width": 20, "height": 1},
        #         {"width": 21, "height": 1},
        #         {"width": 22, "height": 1},
        #         {"width": 28, "height": 1},
        #         {"width": 38, "height": 1},
        #         {"width": 126, "height": 1},
        #         {"width": 1, "height": 126}
        #     ],
        #     "locations": [
        #     {"x": 1, "y": 1, "Rec": 46},
        #     {"x": 1, "y": 2, "Rec": 6},
        #     {"x": 2, "y": 8, "Rec": 36},
        #     {"x": 2, "y": 19, "Rec": 30},
        #     {"x": 2, "y": 20, "Rec": 20},
        #     {"x": 3, "y": 42, "Rec": 33},
        #     {"x": 3, "y": 52, "Rec": 32},
        #     {"x": 3, "y": 53, "Rec": 12},
        #     {"x": 3, "y": 89, "Rec": 23},
        #     {"x": 3, "y": 90, "Rec": 18},
        #     {"x": 3, "y": 112, "Rec": 12},
        #     {"x": 4, "y": 9, "Rec": 34},
        #     {"x": 4, "y": 10, "Rec": 3},
        #     {"x": 4, "y": 65, "Rec": 43},
        #     {"x": 4, "y": 108, "Rec": 4},
        #     {"x": 4, "y": 124, "Rec": 43},
        #     {"x": 5, "y": 13, "Rec": 6},
        #     {"x": 5, "y": 69, "Rec": 49},
        #     {"x": 5, "y": 70, "Rec": 19},
        #     {"x": 7, "y": 43, "Rec": 9},
        #     {"x": 7, "y": 108, "Rec": 47},
        #     {"x": 7, "y": 109, "Rec": 3},
        #     {"x": 8, "y": 112, "Rec": 39},
        #     {"x": 9, "y": 2, "Rec": 26},
        #     {"x": 9, "y": 7, "Rec": 35},
        #     {"x": 10, "y": 13, "Rec": 32},
        #     {"x": 10, "y": 14, "Rec": 5},
        #     {"x": 11, "y": 5, "Rec": 0},
        #     {"x": 11, "y": 19, "Rec": 34},
        #     {"x": 11, "y": 42, "Rec": 33},
        #     {"x": 11, "y": 43, "Rec": 9},
        #     {"x": 12, "y": 4, "Rec": 24},
        #     {"x": 12, "y": 14, "Rec": 23},
        #     {"x": 12, "y": 15, "Rec": 2},
        #     {"x": 12, "y": 52, "Rec": 33},
        #     {"x": 12, "y": 76, "Rec": 43},
        #     {"x": 12, "y": 77, "Rec": 12},
        #     {"x": 13, "y": 17, "Rec": 43},
        #     {"x": 13, "y": 89, "Rec": 40},
        #     {"x": 14, "y": 10, "Rec": 26},
        #     {"x": 15, "y": 2, "Rec": 3},
        #     {"x": 16, "y": 5, "Rec": 1},
        #     {"x": 16, "y": 9, "Rec": 24},
        #     {"x": 16, "y": 20, "Rec": 6},
        #     {"x": 16, "y": 33, "Rec": 36},
        #     {"x": 16, "y": 34, "Rec": 8},
        #     {"x": 16, "y": 53, "Rec": 12},
        #     {"x": 16, "y": 113, "Rec": 2},
        #     {"x": 16, "y": 119, "Rec": 36},
        #     {"x": 16, "y": 120, "Rec": 4},
        #     {"x": 17, "y": 1, "Rec": 48},
        #     {"x": 17, "y": 2, "Rec": 4},
        #     {"x": 17, "y": 26, "Rec": 33},
        #     {"x": 17, "y": 39, "Rec": 33},
        #     {"x": 17, "y": 40, "Rec": 17},
        #     {"x": 17, "y": 115, "Rec": 33},
        #     {"x": 18, "y": 48, "Rec": 32},
        #     {"x": 18, "y": 57, "Rec": 32},
        #     {"x": 22, "y": 21, "Rec": 23},
        #     {"x": 22, "y": 22, "Rec": 4},
        #     {"x": 22, "y": 34, "Rec": 8},
        #     {"x": 22, "y": 44, "Rec": 29},
        #     {"x": 22, "y": 53, "Rec": 27},
        #     {"x": 22, "y": 111, "Rec": 43},
        #     {"x": 22, "y": 112, "Rec": 3},
        #     {"x": 22, "y": 120, "Rec": 2},
        #     {"x": 23, "y": 38, "Rec": 24},
        #     {"x": 23, "y": 40, "Rec": 2},
        #     {"x": 23, "y": 58, "Rec": 11},
        #     {"x": 23, "y": 77, "Rec": 21},
        #     {"x": 23, "y": 122, "Rec": 40},
        #     {"x": 24, "y": 18, "Rec": 3},
        #     {"x": 24, "y": 89, "Rec": 44},
        #     {"x": 26, "y": 38, "Rec": 51},
        #     {"x": 26, "y": 39, "Rec": 38},
        #     {"x": 27, "y": 40, "Rec": 25},
        #     {"x": 27, "y": 44, "Rec": 29},
        #     {"x": 27, "y": 53, "Rec": 27},
        #     {"x": 27, "y": 58, "Rec": 13},
        #     {"x": 27, "y": 75, "Rec": 30},
        #     {"x": 27, "y": 76, "Rec": 9},
        #     {"x": 28, "y": 17, "Rec": 33},
        #     {"x": 28, "y": 18, "Rec": 3},
        #     {"x": 28, "y": 71, "Rec": 0},
        #     {"x": 28, "y": 85, "Rec": 32},
        #     {"x": 29, "y": 21, "Rec": 34},
        #     {"x": 29, "y": 48, "Rec": 32},
        #     {"x": 29, "y": 57, "Rec": 32},
        #     {"x": 29, "y": 67, "Rec": 40},
        #     {"x": 29, "y": 68, "Rec": 3},
        #     {"x": 29, "y": 76, "Rec": 39},
        #     {"x": 31, "y": 93, "Rec": 39},
        #     {"x": 31, "y": 94, "Rec": 9},
        #     {"x": 32, "y": 78, "Rec": 52},
        #     {"x": 32, "y": 79, "Rec": 6},
        #     {"x": 32, "y": 103, "Rec": 36},
        #     {"x": 33, "y": 2, "Rec": 15},
        #     {"x": 33, "y": 40, "Rec": 17},
        #     {"x": 33, "y": 112, "Rec": 10},
        #     {"x": 34, "y": 51, "Rec": 43},
        #     {"x": 34, "y": 52, "Rec": 13},
        #     {"x": 35, "y": 5, "Rec": 54},
        #     {"x": 35, "y": 6, "Rec": 15},
        #     {"x": 35, "y": 65, "Rec": 40},
        #     {"x": 37, "y": 82, "Rec": 32},
        #     {"x": 37, "y": 83, "Rec": 6},
        #     {"x": 37, "y": 107, "Rec": 30},
        #     {"x": 37, "y": 108, "Rec": 17},
        #     {"x": 38, "y": 68, "Rec": 8},
        #     {"x": 38, "y": 125, "Rec": 47},
        #     {"x": 39, "y": 9, "Rec": 33},
        #     {"x": 39, "y": 10, "Rec": 11},
        #     {"x": 39, "y": 94, "Rec": 32},
        #     {"x": 39, "y": 95, "Rec": 25},
        #     {"x": 39, "y": 101, "Rec": 25},
        #     {"x": 40, "y": 21, "Rec": 32},
        #     {"x": 40, "y": 83, "Rec": 8},
        #     {"x": 40, "y": 104, "Rec": 3},
        #     {"x": 41, "y": 10, "Rec": 30},
        #     {"x": 41, "y": 11, "Rec": 8},
        #     {"x": 41, "y": 91, "Rec": 30},
        #     {"x": 41, "y": 106, "Rec": 42},
        #     {"x": 42, "y": 19, "Rec": 34},
        #     {"x": 43, "y": 22, "Rec": 6},
        #     {"x": 43, "y": 92, "Rec": 2},
        #     {"x": 44, "y": 28, "Rec": 32},
        #     {"x": 45, "y": 52, "Rec": 3},
        #     {"x": 45, "y": 61, "Rec": 27},
        #     {"x": 46, "y": 43, "Rec": 44},
        #     {"x": 46, "y": 44, "Rec": 11},
        #     {"x": 46, "y": 66, "Rec": 7},
        #     {"x": 47, "y": 9, "Rec": 45},
        #     {"x": 47, "y": 10, "Rec": 23},
        #     {"x": 47, "y": 29, "Rec": 9},
        #     {"x": 47, "y": 73, "Rec": 44},
        #     {"x": 47, "y": 91, "Rec": 34},
        #     {"x": 47, "y": 92, "Rec": 2},
        #     {"x": 48, "y": 11, "Rec": 8},
        #     {"x": 48, "y": 94, "Rec": 34},
        #     {"x": 52, "y": 11, "Rec": 49},
        #     {"x": 52, "y": 12, "Rec": 19},
        #     {"x": 52, "y": 34, "Rec": 36},
        #     {"x": 52, "y": 35, "Rec": 5},
        #     {"x": 53, "y": 23, "Rec": 37},
        #     {"x": 53, "y": 31, "Rec": 33},
        #     {"x": 53, "y": 40, "Rec": 33},
        #     {"x": 53, "y": 79, "Rec": 3},
        #     {"x": 53, "y": 86, "Rec": 36},
        #     {"x": 53, "y": 87, "Rec": 4},
        #     {"x": 53, "y": 95, "Rec": 4},
        #     {"x": 54, "y": 82, "Rec": 33},
        #     {"x": 54, "y": 99, "Rec": 46},
        #     {"x": 54, "y": 117, "Rec": 45},
        #     {"x": 54, "y": 118, "Rec": 7},
        #     {"x": 58, "y": 25, "Rec": 24},
        #     {"x": 58, "y": 28, "Rec": 26},
        #     {"x": 58, "y": 35, "Rec": 1},
        #     {"x": 58, "y": 38, "Rec": 25},
        #     {"x": 59, "y": 32, "Rec": 4},
        #     {"x": 59, "y": 41, "Rec": 2},
        #     {"x": 59, "y": 74, "Rec": 8},
        #     {"x": 59, "y": 87, "Rec": 6},
        #     {"x": 60, "y": 10, "Rec": 44},
        #     {"x": 60, "y": 93, "Rec": 46},
        #     {"x": 63, "y": 23, "Rec": 34},
        #     {"x": 63, "y": 24, "Rec": 19},
        #     {"x": 63, "y": 73, "Rec": 45},
        #     {"x": 63, "y": 74, "Rec": 5},
        #     {"x": 63, "y": 81, "Rec": 25},
        #     {"x": 63, "y": 84, "Rec": 4},
        #     {"x": 64, "y": 31, "Rec": 34},
        #     {"x": 64, "y": 32, "Rec": 6},
        #     {"x": 64, "y": 43, "Rec": 46},
        #     {"x": 64, "y": 76, "Rec": 34},
        #     {"x": 64, "y": 77, "Rec": 2},
        #     {"x": 64, "y": 88, "Rec": 40},
        #     {"x": 65, "y": 38, "Rec": 32},
        #     {"x": 65, "y": 83, "Rec": 32},
        #     {"x": 68, "y": 100, "Rec": 17},
        #     {"x": 69, "y": 12, "Rec": 3},
        #     {"x": 69, "y": 19, "Rec": 40},
        #     {"x": 69, "y": 20, "Rec": 3},
        #     {"x": 69, "y": 32, "Rec": 32},
        #     {"x": 69, "y": 36, "Rec": 43},
        #     {"x": 69, "y": 37, "Rec": 1},
        #     {"x": 69, "y": 77, "Rec": 6},
        #     {"x": 70, "y": 15, "Rec": 23},
        #     {"x": 71, "y": 110, "Rec": 43},
        #     {"x": 71, "y": 111, "Rec": 10},
        #     {"x": 72, "y": 4, "Rec": 41},
        #     {"x": 72, "y": 11, "Rec": 4},
        #     {"x": 72, "y": 121, "Rec": 40},
        #     {"x": 73, "y": 14, "Rec": 31},
        #     {"x": 73, "y": 21, "Rec": 50},
        #     {"x": 73, "y": 22, "Rec": 10},
        #     {"x": 74, "y": 84, "Rec": 44},
        #     {"x": 74, "y": 85, "Rec": 3},
        #     {"x": 74, "y": 94, "Rec": 2},
        #     {"x": 75, "y": 96, "Rec": 41},
        #     {"x": 77, "y": 44, "Rec": 28},
        #     {"x": 77, "y": 50, "Rec": 6},
        #     {"x": 77, "y": 61, "Rec": 36},
        #     {"x": 77, "y": 62, "Rec": 11},
        #     {"x": 78, "y": 25, "Rec": 44},
        #     {"x": 78, "y": 26, "Rec": 6},
        #     {"x": 78, "y": 42, "Rec": 34},
        #     {"x": 78, "y": 56, "Rec": 36},
        #     {"x": 78, "y": 100, "Rec": 40},
        #     {"x": 78, "y": 101, "Rec": 7},
        #     {"x": 79, "y": 14, "Rec": 30},
        #     {"x": 79, "y": 15, "Rec": 4},
        #     {"x": 79, "y": 32, "Rec": 30},
        #     {"x": 79, "y": 49, "Rec": 32},
        #     {"x": 79, "y": 108, "Rec": 38},
        #     {"x": 81, "y": 33, "Rec": 3},
        #     {"x": 82, "y": 5, "Rec": 9},
        #     {"x": 82, "y": 62, "Rec": 6},
        #     {"x": 82, "y": 111, "Rec": 3},
        #     {"x": 82, "y": 118, "Rec": 53},
        #     {"x": 82, "y": 119, "Rec": 2},
        #     {"x": 83, "y": 43, "Rec": 23},
        #     {"x": 83, "y": 46, "Rec": 30},
        #     {"x": 83, "y": 47, "Rec": 2},
        #     {"x": 83, "y": 68, "Rec": 32},
        #     {"x": 83, "y": 114, "Rec": 33},
        #     {"x": 85, "y": 38, "Rec": 33},
        #     {"x": 85, "y": 39, "Rec": 4},
        #     {"x": 85, "y": 47, "Rec": 9},
        #     {"x": 85, "y": 61, "Rec": 40},
        #     {"x": 85, "y": 69, "Rec": 23},
        #     {"x": 85, "y": 70, "Rec": 10},
        #     {"x": 86, "y": 80, "Rec": 30},
        #     {"x": 86, "y": 85, "Rec": 4},
        #     {"x": 86, "y": 93, "Rec": 30},
        #     {"x": 86, "y": 94, "Rec": 2},
        #     {"x": 87, "y": 62, "Rec": 7},
        #     {"x": 87, "y": 89, "Rec": 0},
        #     {"x": 87, "y": 101, "Rec": 2},
        #     {"x": 87, "y": 105, "Rec": 26},
        #     {"x": 88, "y": 26, "Rec": 30},
        #     {"x": 88, "y": 27, "Rec": 7},
        #     {"x": 88, "y": 69, "Rec": 32},
        #     {"x": 88, "y": 81, "Rec": 8},
        #     {"x": 88, "y": 94, "Rec": 9},
        #     {"x": 88, "y": 109, "Rec": 5},
        #     {"x": 89, "y": 34, "Rec": 38},
        #     {"x": 89, "y": 39, "Rec": 36},
        #     {"x": 89, "y": 40, "Rec": 24},
        #     {"x": 89, "y": 44, "Rec": 25},
        #     {"x": 89, "y": 47, "Rec": 9},
        #     {"x": 90, "y": 56, "Rec": 0},
        #     {"x": 91, "y": 8, "Rec": 41},
        #     {"x": 91, "y": 9, "Rec": 9},
        #     {"x": 91, "y": 46, "Rec": 32},
        #     {"x": 91, "y": 49, "Rec": 34},
        #     {"x": 91, "y": 50, "Rec": 6},
        #     {"x": 92, "y": 18, "Rec": 23},
        #     {"x": 92, "y": 56, "Rec": 23},
        #     {"x": 92, "y": 62, "Rec": 7},
        #     {"x": 93, "y": 19, "Rec": 2},
        #     {"x": 93, "y": 25, "Rec": 38},
        #     {"x": 93, "y": 26, "Rec": 33},
        #     {"x": 93, "y": 68, "Rec": 32},
        #     {"x": 93, "y": 69, "Rec": 30},
        #     {"x": 93, "y": 80, "Rec": 23},
        #     {"x": 93, "y": 81, "Rec": 16},
        #     {"x": 93, "y": 103, "Rec": 43},
        #     {"x": 93, "y": 104, "Rec": 10},
        #     {"x": 94, "y": 97, "Rec": 40},
        #     {"x": 94, "y": 114, "Rec": 0},
        #     {"x": 95, "y": 40, "Rec": 6},
        #     {"x": 95, "y": 56, "Rec": 0},
        #     {"x": 95, "y": 70, "Rec": 10},
        #     {"x": 95, "y": 106, "Rec": 39},
        #     {"x": 95, "y": 107, "Rec": 7},
        #     {"x": 96, "y": 50, "Rec": 6},
        #     {"x": 96, "y": 61, "Rec": 33},
        #     {"x": 96, "y": 114, "Rec": 30},
        #     {"x": 97, "y": 27, "Rec": 7},
        #     {"x": 97, "y": 56, "Rec": 30},
        #     {"x": 97, "y": 62, "Rec": 6},
        #     {"x": 98, "y": 18, "Rec": 30},
        #     {"x": 98, "y": 19, "Rec": 2},
        #     {"x": 99, "y": 21, "Rec": 34},
        #     {"x": 100, "y": 26, "Rec": 7},
        #     {"x": 100, "y": 41, "Rec": 23},
        #     {"x": 100, "y": 42, "Rec": 14},
        #     {"x": 100, "y": 62, "Rec": 14},
        #     {"x": 100, "y": 114, "Rec": 32},
        #     {"x": 101, "y": 9, "Rec": 9},
        #     {"x": 101, "y": 33, "Rec": 23},
        #     {"x": 101, "y": 76, "Rec": 38},
        #     {"x": 102, "y": 34, "Rec": 7},
        #     {"x": 102, "y": 82, "Rec": 34},
        #     {"x": 102, "y": 83, "Rec": 6},
        #     {"x": 103, "y": 89, "Rec": 32},
        #     {"x": 104, "y": 95, "Rec": 32},
        #     {"x": 104, "y": 96, "Rec": 1},
        #     {"x": 104, "y": 104, "Rec": 10},
        #     {"x": 105, "y": 8, "Rec": 33},
        #     {"x": 105, "y": 9, "Rec": 12},
        #     {"x": 105, "y": 25, "Rec": 32},
        #     {"x": 105, "y": 26, "Rec": 2},
        #     {"x": 105, "y": 106, "Rec": 40},
        #     {"x": 105, "y": 114, "Rec": 32},
        #     {"x": 106, "y": 28, "Rec": 34},
        #     {"x": 106, "y": 33, "Rec": 36},
        #     {"x": 106, "y": 34, "Rec": 7},
        #     {"x": 107, "y": 41, "Rec": 47},
        #     {"x": 107, "y": 83, "Rec": 1},
        #     {"x": 107, "y": 87, "Rec": 25},
        #     {"x": 108, "y": 77, "Rec": 7},
        #     {"x": 108, "y": 90, "Rec": 5},
        #     {"x": 109, "y": 12, "Rec": 27},
        #     {"x": 109, "y": 17, "Rec": 8},
        #     {"x": 109, "y": 111, "Rec": 45},
        #     {"x": 109, "y": 112, "Rec": 2},
        #     {"x": 109, "y": 119, "Rec": 4},
        #     {"x": 110, "y": 4, "Rec": 45},
        #     {"x": 110, "y": 5, "Rec": 3},
        #     {"x": 110, "y": 123, "Rec": 43},
        #     {"x": 111, "y": 16, "Rec": 43},
        #     {"x": 112, "y": 25, "Rec": 41},
        #     {"x": 112, "y": 26, "Rec": 2},
        #     {"x": 112, "y": 34, "Rec": 1},
        #     {"x": 112, "y": 76, "Rec": 40},
        #     {"x": 112, "y": 77, "Rec": 7},
        #     {"x": 112, "y": 87, "Rec": 25},
        #     {"x": 112, "y": 90, "Rec": 5},
        #     {"x": 113, "y": 35, "Rec": 39},
        #     {"x": 113, "y": 82, "Rec": 34},
        #     {"x": 113, "y": 83, "Rec": 1},
        #     {"x": 113, "y": 95, "Rec": 30},
        #     {"x": 114, "y": 89, "Rec": 32},
        #     {"x": 115, "y": 96, "Rec": 10},
        #     {"x": 118, "y": 83, "Rec": 6},
        #     {"x": 122, "y": 26, "Rec": 9},
        #     {"x": 122, "y": 42, "Rec": 22},
        #     {"x": 122, "y": 112, "Rec": 11},
        #     {"x": 123, "y": 5, "Rec": 11},
        #     {"x": 1, "y": 1, "Rec": 55},
        #     {"x": 1, "y": 1, "Rec": 56},
        #     {"x": 126, "y": 1, "Rec": 56},
        #     {"x": 1, "y": 126, "Rec": 55}
        #     ]
        # },
        "enemies": {
            # "EnemyPositions": [
            #     [
            #         {"x": 6, "y": 24},
            #         {"x": 13, "y": 23},
            #         {"x": 13, "y": 36},
            #         {"x": 14, "y": 30}
            #     ],
            #     [
            #         {"x": 9, "y": 73},
            #         {"x": 9, "y": 82},
            #         {"x": 19, "y": 73}
            #     ],
            #     [
            #         {"x": 10, "y": 99},
            #         {"x": 14, "y": 94},
            #         {"x": 19, "y": 100}
            #     ],
            #     [
            #         {"x": 29, "y": 114}
            #     ],
            #     [
            #         {"x": 22, "y": 8},
            #         {"x": 27, "y": 5},
            #         {"x": 28, "y": 13}
            #     ],
            #     [
            #         {"x": 27, "y": 26},
            #         {"x": 30, "y": 32},
            #         {"x": 38, "y": 26},
            #         {"x": 39, "y": 32}
            #     ],
            #     [
            #         {"x": 19, "y": 43}
            #     ],
            #     [
            #         {"x": 45, "y": 82},
            #         {"x": 50, "y": 81},
            #         {"x": 50, "y": 87}
            #     ],
            #     [
            #         {"x": 41, "y": 99},
            #         {"x": 45, "y": 96},
            #         {"x": 64, "y": 103},
            #         {"x": 64, "y": 111}
            #     ],
            #     [
            #         {"x": 58, "y": 15},
            #         {"x": 58, "y": 19},
            #         {"x": 65, "y": 14},
            #         {"x": 65, "y": 20}
            #     ],
            #     [
            #         {"x": 52, "y": 48},
            #         {"x": 53, "y": 67},
            #         {"x": 72, "y": 48},
            #         {"x": 72, "y": 68}
            #     ],
            #     [
            #         {"x": 74, "y": 112},
            #         {"x": 74, "y": 118},
            #         {"x": 79, "y": 112},
            #         {"x": 79, "y": 118}
            #     ],
            #     [
            #         {"x": 69, "y": 34},
            #         {"x": 76, "y": 34}
            #     ],
            #     [
            #         {"x": 96, "y": 12}
            #     ],
            #     [
            #         {"x": 117, "y": 28},
            #         {"x": 116, "y": 32}
            #     ],
            #     [
            #         {"x": 78, "y": 58}
            #     ],
            #     [
            #         {"x": 94, "y": 52}
            #     ],
            #     [
            #         {"x": 94, "y": 64}
            #     ],
            #     [
            #         {"x": 107, "y": 46},
            #         {"x": 118, "y": 45},
            #         {"x": 107, "y": 54},
            #         {"x": 118, "y": 54}
            #     ],
            #     [
            #         {"x": 107, "y": 62},
            #         {"x": 118, "y": 62},
            #         {"x": 107, "y": 70},
            #         {"x": 118, "y": 70}
            #     ],
            #     [
            #         {"x": 91, "y": 91},
            #         {"x": 91, "y": 100},
            #         {"x": 84, "y": 116}
            #     ],
            #     [
            #         {"x": 107, "y": 98},
            #         {"x": 107, "y": 103},
            #         {"x": 110, "y": 100}
            #     ],
            #     [
            #         {"x": 101, "y": 110}
            #     ]
            # ],
            "EnemyScenes": ["res://Scenes/Characters/Enemies/EnemySkelly.tscn", "res://Scenes/Characters/Enemies/EnemySkelly2.tscn"]
        },
        "difficulty": {"Easy": {
            "Min_Enemies_Per_Area": 1,
            "Max_Enemies_Per_Area": 2,
            #Will need tailoring
            "Enemy_Stats": [
                {"Exp": 2, "Health": 20, "Mana": 0, "Mana_Regen": 0.0, "Defence": 1, "Damage": 9, "Speed": 1.5, "Poison_Resist": 0.5, "Magic_Resist": 0.2, "Fire_Resist": 0.2, "Cold_Resist": 0.2, "Lightning_Resist": 0.2},
                {"Exp": 2, "Health": 25, "Mana": 0, "Mana_Regen": 0.0, "Defence": 2, "Damage": 10, "Speed": 1.0, "Poison_Resist": 0.5, "Magic_Resist": 0.2, "Fire_Resist": 0.2, "Cold_Resist": 0.2, "Lightning_Resist": 0.2}
            ],
            "Loot_rating": [50, 150], # Lower Bound, Upper Bound
            "Rarity_Distribution": [[0, 100], [100, 150], [150, 250], [200, 300], [250, 300]], # Common, Uncommon, Rare, Epic, Legendary
            "Enemy_item_count": [0, 2] # Min, Max
        },
        "Medium": 2, "Hard": 3, "Insane": 4, "Legendary": 5
        },
        "Spawnpoints": [
            {"x": 4, "y": 3},
            {"x": 11, "y": 119},
            {"x": 13, "y": 62},
            {"x": 30, "y": 53},
            {"x": 35, "y": 68},
            {"x": 39, "y": 108},
            {"x": 42, "y": 62},
            {"x": 45, "y": 14},
            {"x": 54, "y": 27},
            {"x": 66, "y": 80},
            {"x": 81, "y": 103},
            {"x": 84, "y": 64},
            {"x": 92, "y": 70},
            {"x": 103, "y": 83},
            {"x": 111, "y": 5},
            {"x": 119, "y": 120}
        ]
    }
}

var map_path = ""
var current_difficulty = 0
var player_base_stats = {}

func set_map_path(path: String) -> void:
    map_path = path

func set_difficulty(difficulty: int) -> void:
    current_difficulty = difficulty

func set_player_base_stats(stats: Dictionary) -> void:
    player_base_stats = stats